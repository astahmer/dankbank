...
export const useVelocityTrackedSpring = (initialConfigFunc: () => ObjectData, _trackedVar?: string) => {
    ...
    const wrappedSet = (
        data: ObjectData,
        { skipTrackVelocity, skipSetVelocity }: UseVelocityTrackedSpringOptions = {}
    ) => {
        // update velocity tracker
        const velocityTrackerArgs = { config: data.config } as any;
        if (data[trackedVar] && !skipTrackVelocity) velocityTrackerArgs.velocityTracker = data[trackedVar];
    ...
    return [springVals, wrappedSet] as const;
};

type UseVelocityTrackedSpringOptions = { skipTrackVelocity?: boolean; skipSetVelocity?: boolean };
type ObjectData = Record<string, any>;

const getTrackedVar = (_trackedVar: string, initialConfig: ObjectData) => {
